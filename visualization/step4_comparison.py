"""
Compare gistemp5 vs gistemp4.0 step 4 (ocean) output.

Requires cached outputs. Run from repo root:
    python main/run.py                  # generates cache/step4_1880_2026.parquet
    python testing/compare_step4.py     # generates gistemp4.0/tmp/step4_cache.parquet
Then:
    python visualization/step4_comparison.py
"""
...
V4_CACHE = os.path.join(REPO_ROOT, '..', 'gistemp4.0', 'tmp', 'step4_cache.parquet')
OUT_PATH  = os.path.join(REPO_ROOT, 'visualization', 'step4_comparison.png')
...
def load_data():
    if not os.path.exists(V4_CACHE):
        raise FileNotFoundError(
            f"gistemp4.0 step4 cache missing: {V4_CACHE}\n"
            "Run: python testing/compare_step4.py"
        )
    df4 = pd.read_parquet(V4_CACHE)
    df5 = step_cache.load('step4', START_YEAR, END_YEAR)
    if df5 is None:
        raise FileNotFoundError(
            f"gistemp5 step4 cache missing.\nRun: python main/run.py"
        )
    return df4, df5
...
def annual_global_mean(df):
    ...
    for yr in years:
        ...
        vals = df[cols].values
        valid = ~np.isnan(vals)
        means.append(float(vals[valid].mean()) if valid.any() else np.nan)
        # count subboxes with any data this year
        active.append(int(valid.any(axis=1).sum()))
...
def validate(df4, df5):
    ...
    tc5_list = time_cols(df5)
    ocean5 = int((~df5[tc5_list].isna().all(axis=1)).sum())
    tc4_list = time_cols(df4)
    ocean4 = int((~df4[tc4_list].isna().all(axis=1)).sum())
    return {
...
def plot(df4, df5, stats):
    ...
    ax.set_ylabel('Mean SST Anomaly (°C)', fontsize=11)
    ax.set_title('Global Mean Ocean SST Anomaly (unweighted mean across ocean subboxes)', fontsize=12)
    ...
    # Panel 3: active ocean subbox count per year
    ax = axes[2]
    ...
    ax.set_ylabel('Active Ocean Subboxes', fontsize=11)
    ax.set_xlabel('Year', fontsize=11)
    ax.set_title(
        f'Ocean Subbox Coverage per Year  '
        f'[ocean subboxes: v4={stats["ocean4"]:,}, v5={stats["ocean5"]:,}]',
        fontsize=12
    )
    ax.legend(fontsize=10)
    ax.grid(alpha=0.25)
...
if __name__ == '__main__':
    ...
    print(f"  Cells compared         : {stats['cells_compared']:,}")
    ...
    print(f"  Ocean subboxes v4/v5   : {stats['ocean4']:,} / {stats['ocean5']:,}")
    print("Plotting...")
    plot(df4, df5, stats)
