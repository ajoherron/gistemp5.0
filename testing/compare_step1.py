"""
Compare gistemp5 step1 output against gistemp4.0 step1.

Run from repo root:
    python testing/compare_step1.py
"""
...
V4_CACHE    = os.path.join(V4_DIR, 'tmp', 'step1_cache.parquet')
DUMP_SCRIPT = os.path.join(REPO_ROOT, 'testing', '_v4_step1_dump.py')
...
def run_v4_step1(force: bool = False) -> pd.DataFrame:
    ...
    if cache_fresh:
        print("  Loading cached gistemp4.0 step1 output …")
        return pd.read_parquet(V4_CACHE)

    print("  Running gistemp4.0 step0 → step1 (this takes a few minutes) …")
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError("gistemp4.0 step1 dump failed")
...
def compare(df5: pd.DataFrame, df4: pd.DataFrame):
    ...
    n_differ = (abs_diff[both_valid] > 1e-6).sum().sum()
...
def main():
    ...
    parser.add_argument('--force', action='store_true', help='Ignore cache and re-run v4 step1')
    args = parser.parse_args()

    print("=== Step 1 comparison: gistemp5 vs gistemp4.0 ===\n")

    print("[1/4] Fetching additional gistemp4.0 input data")
    fetch_v4_inputs()

    print("\n[2/4] Running gistemp4.0 step0 → step1")
    df4 = run_v4_step1(force=args.force)

    print("\n[3/4] Running gistemp5 step0")
    df0 = step0(GHCN_TEMP_URL, GHCN_META_URL, START_YEAR, END_YEAR)

    print("\n[4/4] Running gistemp5 step1")
    df5 = step1(df0, STRANGE_URL, START_YEAR, END_YEAR)
